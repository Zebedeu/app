...
exports.list = function(req, res) {
	res.render('producers/statistics/total_sales', {
		user: {
		...
		breadcrumb: "<li class='breadcrumb-item'><a href='"+config.base_url+"producers/dashboard'>"+labels['LBL_HOME'][(req.session.language || config.default_language_code)]+"</a></li><li class='breadcrumb-item active' aria-current='page'>"+labels['LBL_PRODUCER_DASHBOARD_STATISTICS_TOTAL_SALES'][(req.session.language || config.default_language_code)]+"</li>",
...
exports.totalSales = function(req, res) {
	...
	Order.find({ 'products.user_info.user_id': {$in: [req.session.user_id]}, status: { $in: ['delivered'] } }, { _id: 0, updated_at: 0 }, (err, orders) => {
