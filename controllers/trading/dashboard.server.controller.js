...
exports.matchProducts = (req, res) => {
	Product.aggregate([
		...
		Product.aggregate([
			...
			let categories = [], catids = [], unit_prices = [], demandArr = [], over_margin = req.session.over_margin;
			...
			if (products.length > 0) {
				_.each(products, (element, index, list) => {
					...
					if (element.remaining_unit_value >= element.min_qty) {
						...
						tbl_recently_viewed_products += "<tr class='" + className + "' onClick=productDetails('" + config.base_url + "','trading','" + element.product_id + "')><td class='capitalize'>" + element.sub_category_title + "</td><td class='capitalize'>" + element.category_title + "</td><td>Kz " + separators(element.unit_price) + ' / ' + element.unit_type + "</td><td>" + separatorsWD(element.remaining_unit_value) + ' ' + (parseFloat(element.remaining_unit_value) > 1 ? element.unit_plural_title : element.unit_type) + "</td><td>" + element.state_name + "</td></tr>";
					}
				})
			} else {
	...
	Order.find({ 'buyer_info.user_id': { $in: [req.session.user_id] }, status: { $nin: ['cancelled', 'completed', 'shipped'] } }, { _id: 0, updated_at: 0 }, (err, orders) => {
		...
		if (orders.length > 0) {
			_.each(orders, (element, index, list) => {
				...
				tbl_ongoing_orders += "<tr class='" + className + "' onClick=orderDetails('" + config.base_url + "','trading','" + element.order_id + "')><td>" + element.order_id + "</td><td>" + ordered_date + "</td><td>" + shipped_date + "</td><td>" + address + "</td><td>Kz " + separators(element.total) + "</td><td style='text-transform:capitalize;'>" + element.status + "</td></tr>";
			})
		} else {
