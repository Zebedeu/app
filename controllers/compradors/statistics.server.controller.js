	...
	Order.find({ 'buyer_info.user_id': {$in: [req.session.user_id]}, status: { $nin: ['cancelled'] } }, { _id: 0, updated_at: 0 }, (err, orders) => {
		...
		if(orders.length > 0){
			_.each(orders, (element, index, list) => {
				...
				tbl_purchase_orders+= "<tr class='"+className+"' onClick=orderDetails('"+config.base_url+"','compradors','"+element.order_id+"')><td>"+element.order_id+"</td><td>"+ordered_date+"</td><td>"+shipped_date+"</td><td>"+(element.address_info.locality+', '+element.address_info.city_district+', '+element.address_info.state)+"</td><td>"+separators(element.total)+" Kz</td><td style='text-transform:capitalize;'>"+_status+"</td></tr>";
			})
