                         ...
                         Cart.remove({ user_id: req.session.user_id }, (err, remove_response) => {
                              Sms_template.find({ code: { $in: ['ORDER_PLACED', 'SELLER_NEW_ORDER'] } }, { _id: 0, sms_template_id: 1, title: 1, code: 1, value: 1 }, (err, sms) => {
                                   ...
                                   _.each(products, (element, index, list) => {
                                        ...
                                        productStr += "<tr><td style='width: 15%;'><img src='" + productImg + "' style='width: 100%;'></td><td style='vertical-align: middle;'>" + element.category_title[req.session.language] + " - " + element.sub_category_title[req.session.language] + "</td><td style='font-size: 14px;vertical-align: middle;'>Quantity : " + element.qty + "</td><td style='font-size: 20px;vertical-align: middle;'>Kz " + (parseFloat(element.unit_price)).toFixed(2) + "/" + element.unit_type[req.session.language] + "</td><td style='font-size: 20px;vertical-align: middle;'>Kz " + (parseFloat(element.qty * element.unit_price)).toFixed(2) + "</td></tr>";
                                   })
...
exports.list = async (req, res) => {
     ...
     Cart.aggregate([
          ...
          res.render('trading/checkout/list', {
               user: {
                    user_id: req.session.user_id,
...
updateWalletBalance = (requestParam, payment_type) => {
     ...
     function updateData(value) {
          return new Promise(async (resolve, reject) => {
               let columnAndValues = {};
               if (value.user_type == 'trading' && payment_type == 'wallet') {
                    columnAndValues = {
                         $inc: {
