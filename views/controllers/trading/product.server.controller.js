...
exports.filterList = function(req, res) {
	...
	Product.aggregate([
		...
		if(products.length == 0){
			res.render('trading/product/filter-empty', {
				user: {
			...
			res.render('trading/product/filter-list', {
				user: {
					user_id: req.session.user_id,
					name: req.session.name,
					user_type: req.session.user_type,
...
exports.list = function(req, res) {
	Product.aggregate([
		...
		if(products.length == 0){
			res.render('trading/product/empty', {
				user: {
					user_id: req.session.user_id,
					name: req.session.name,
					user_type: req.session.user_type,
				...
				labels,
				language: req.session.language || config.default_language_code,
				breadcrumb: "<li class='breadcrumb-item'><a href='"+config.base_url+"trading/dashboard'>"+labels['LBL_HOME'][(req.session.language || config.default_language_code)]+"</a></li><li class='breadcrumb-item active' aria-current='page'>"+labels['LBL_EMPTY_DEMAND'][(req.session.language || config.default_language_code)]+"</li>",
			...
			res.render('trading/product/list', {
				user: {
					user_id: req.session.user_id,
					name: req.session.name,
					user_type: req.session.user_type,
				...
				labels,
				language: req.session.language || config.default_language_code,
				breadcrumb: "<li class='breadcrumb-item'><a href='"+config.base_url+"trading/dashboard'>"+labels['LBL_HOME'][(req.session.language || config.default_language_code)]+"</a></li><li class='breadcrumb-item active' aria-current='page'>"+labels['LBL_LIST_DEMAND'][(req.session.language || config.default_language_code)]+"</li>",
				messages : req.flash('error') || req.flash('info'),
				messages : req.flash('info'),
			});
		}
...
exports.add = function(req, res) {
	if(req.body.product_category && req.body.title && req.body.unit && req.body.unit_value && req.body.lower_price_range && req.body.higher_price_range && req.body.state_id && req.body.city_id && req.body.location){
		...
		Language.find({ status: 'active', code: (req.session.language || config.default_language_code) }, { _id: 0, language_id: 1, code: 1, title: 1 }, (err, languages) => {
			res.render('trading/product/add', {
				user: {
					user_id: req.session.user_id,
					name: req.session.name,
					user_type: req.session.user_type,
...
exports.remove = function(req, res) {
	Product.remove({ product_id: req.params.id }, (err, response) => {
		return res.redirect(config.base_url+'trading/product/list');
	})
};
	...
	if(req.params.id) {
		Product.findOne({ product_id: req.params.id }, { _id: 0 }, (err, product) => {
			...
			Unit.findOne({ unit_id: product.unit_type }, { _id: 0, unit_id: 1, max_price_range: 1 }, (err, units) => {
				console.log(units);
				Language.find({ status: 'active', code: (req.session.language || config.default_language_code) }, { _id: 0, language_id: 1, code: 1, title: 1 }, (err, languages) => {
					res.render('trading/product/edit', {
						user: {
							user_id: req.session.user_id,
							name: req.session.name,
							user_type: req.session.user_type,
